...
| `council.tool`                | boolean | `true`                                     | Expose the agent-callable `convene` tool. `consult` asks every active reviewer one question in parallel; `plan` runs a full adjudicated council and returns the plan as a brief that never enters plan review. Both spend on every configured role. See [Tools › convene](./tools/convene.md). |
...
`modelRoles.planner` and `modelRoles.adjudicator` are the two council leads. An unassigned `planner` falls back to the `slow` model role; an unassigned `adjudicator` keeps adjudication in your main session, which is the historical behaviour for both. An agent-convened run is the one exception: it cannot adjudicate in your session without deadlocking against its own tool call, so an unassigned `adjudicator` there falls back to the planner's resolved model and warns.
