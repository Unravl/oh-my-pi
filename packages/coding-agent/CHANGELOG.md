...
- Added `/council`, a multi-model planning command. `/council <task>` resolves an ordered roster of models, drafts a plan with the planner, runs one or two independent review rounds in parallel, adjudicates every report into one verdict, and publishes a single agreed plan. `/council status`, `/council cancel`, `/council resume [run-id]`, and `/council config` drive a run. Nothing is spent until the roster is proven: the task, config, round staffing, model assignments, tool support, and credentials are all validated first, the run id, planner model, every member's role and model, and the round count are printed before the first child launches, and a refusal names both what is wrong and how to fix it.
- Added a live Council HUD, transcript mirroring, and same-session recovery. The HUD is a bounded box above the editor with one row per role carrying status, model, effort, attempts, and running spend; `Ctrl+O` expands it into a scrollable pane with warnings and per-role detail, and `Esc` cancels the run. While exactly one agent is working, its assistant text, thinking, and tool calls are mirrored into the transcript through the same renderers Main uses; parallel review rounds stay HUD-only. Kickoff, round start, round settle, cancellation, and the terminal outcome are each recorded once in the transcript, so an interrupted or failed run still shows its per-role stats table, its `history://` links, and the exact `/council resume <run-id>` command after a restart. Resuming reuses the settled rounds' reports and refuses when any immutable input of the original run changed, naming which one moved.
- Added the council roster, its six settings, and Model Hub editing for all of it. `council.members` is an ordered list of reviewer slots assigned to models like any other role; `council.rounds` picks one or two review rounds, and a member may be pinned to a single round. A member that is disabled, or pinned above the configured round count, is parked: it never runs, is never credential-checked, and never marks a run degraded. Two lead roles sit above the roster: `modelRoles.planner` drafts the plan and falls back to the `slow` role, and `modelRoles.adjudicator` moves judging out of your main session into a read-only child. `council.advisor.planner`, `council.advisor.reviewers`, and `council.advisor.adjudicator` attach an advisor to that role's turns without ever widening the child's tool access, and `council.mirrorTranscript` (default `true`) toggles the live mirror. Every reviewer runs one identical brief, with differentiation supplied entirely by the assigned models. Up to 64 reviewers may be active in a round; how many run at once stays governed by `task.maxConcurrency` and provider limits. The Model Hub's `Roles & Council` section edits the lead rows, the per-round reviewer groups, display names, the round count, and the advisor toggles, refuses a change that would exceed 64 active reviewers, and keeps a broken roster editable instead of locking the editor.
- Added session-local plan publication, plan review, stats, and reviewer grading. A finished run publishes exactly one plan as `local://council-<slug>-plan.md`, named deterministically from the task, and creates nothing in the working tree. The plan opens in the normal plan-review overlay with a Council stats box above it: per-role model, effort, status, attempts, cost, findings raised, and the adjudicator's disposition tallies, grouped by round and headed by the run state and total duration. The adjudicator also grades each reviewer's whole contribution `S`/`A`/`B`/`C`/`D`, weighted by the severity, correctness, and evidence quality of what it surfaced rather than by how much it wrote; a reviewer that never finished is graded `F`. Spend is recorded durably as `plannerUsage`, per-member `usage`, and `adjudicatorUsage`.
- Added the `xd://council` write device and non-TUI host support. An adjudicator running in your main session submits its verdict, per-finding dispositions, and reviewer grades by writing to `xd://council`; `read xd://council` returns the schema, and the device refuses with `No council run is awaiting adjudication.` outside an adjudication turn. A delegated adjudicator yields its verdict directly and needs neither the device nor Main's `write` tool. `/council` also runs under the ACP and RPC hosts, which print text guidance where the TUI opens the Model Hub; an RPC client is answered when the run finishes and should correlate responses by id.
- Added the agent-callable `convene` tool, so the council no longer needs an operator to type `/council`. `action: "consult"` puts one question to every active reviewer in parallel and returns their prose answers with a `history://<agent-id>` per reviewer — no planner, no adjudicator, no manifest, and no publication, so a consult is not resumable, is absent from `/council status`, and neither blocks nor is blocked by a `/council` run. `action: "plan"` runs the ordinary durable council and returns the adjudicated plan to the calling agent. Both actions resolve the same roster `/council` does and confine reviewers to the same five-tool read-only slate; both refuse before spending when the roster is unconfigured. `council.tool` (default `true`) removes the tool.
- An agent-convened council run is recorded as such and never enters plan review. Runs now carry a durable `origin` on the manifest; an `agent`-origin run publishes `local://council-<slug>-brief.md` instead of `council-<slug>-plan.md`, which keeps it out of the `*plan.md` plan listing, and the Council controller skips the plan-approval overlay for it — so an agent can convene the council mid-turn without hijacking a turn to show you an approval screen for a plan you did not ask for. Because such a run holds the turn Main would have to finish before adjudicating, its adjudicator is always delegated: with `modelRoles.adjudicator` unassigned it falls back to the planner's already-resolved model and preflight warns, and `/council resume` reads the origin back off the manifest so a resumed agent-origin run keeps that adjudicator.
- An agent-convened council run attaches no advisor to any child — planner, reviewers, and adjudicator alike — regardless of `council.advisor.*`. The `advisor` role is a single shared model, so watching several council children with it correlates outputs whose whole value is independence, manufacturing the agreement the agent would then report as consensus. The operator's toggles keep applying in full to `/council`. A `consult` additionally ignores `round` pins: a `round` is a review-round schedule and a consult runs none, so every enabled member with an assigned model answers rather than being parked as inert. The round rules are now written down in `AGENTS.md` — an omitted `round` means every configured round, so such a member is always in round 1 — and locked by tests on both the dispatch and consult paths.
- Subagent rows in the task widget now name the model behind them. Each row's resolved `<provider>/<model>` (with its `:<level>` suffix when one was set) sits immediately after the context gauge — `MyAgent · 80 🛠 · 30.1%/500K · anthropic/claude-opus:high` — so how full a child's window is and what is filling it read as one pair, and it is suffixed with `++` when a live advisor watches that child's turns. `task.showResolvedModelBadge` now defaults on and gates the whole badge, the badge is truncated in the middle so the tail that disambiguates a model id survives, and the same treatment applies to `agent()` progress rows inside `eval` and to `task` rows in the background-job list. `++` is now one shared marker across every surface that prints a model next to an agent: the status line, the Council pane and stats, the task widget, and job rows.

### Changed

- `Ctrl+O`, `Shift+Up`, and `Shift+Down` now overload conditionally while a council run is live: `Ctrl+O` toggles the Council pane instead of tool-output expansion, and the Shift arrows scroll the expanded pane, still dequeuing queued messages once the pane cannot scroll further. With no live run all three behave exactly as before.
